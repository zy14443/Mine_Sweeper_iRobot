...
% CreateMatlabSim@gmail.com
    ...
    % All units are kg/m/s/rad unless otherwise noted
        radius= 0.1651; %0.2;        % Radius of the robot (model as a circle)
...
%                     [y_sensor y_sensor+dist*sin(th_sensor)])
    ...
    % Sensor Functions
        ...
        function dis2wall = ReadWallSignal(obj)
        % dis2wall = ReadWallSignal(obj)
        % Generates a reading for the infrared wall sensor
        % Modified by Yu, 4/13/2012
        % Input:
        % obj - Instance of class CreateRobot
        %
        % Output:
        % dis2wall - Distance by wall sensor from the wall to the right
        %            edge of the robot 
            
            % Calculate position of sensor
            sensAng= -1.0124; % Sensor placement relative to front of robot
            x_sensor= obj.posAbs(1)+obj.radius*cos(obj.thAbs+sensAng);
            y_sensor= obj.posAbs(2)+obj.radius*sin(obj.thAbs+sensAng);
            dis2edge = obj.radius*(1 - sin(-sensAng)); % distance from wall sensor to right edge. added by Yu. 4/13
            ...
            % Generate reading
            dis2wall = distIR + noiseVal;
            if dis2wall < dis2edge
                dis2wall = 0;
            else
                dis2wall = dis2wall - dis2edge;
            end
        end
